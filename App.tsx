...
const BOOKS_STORE_KEY = 'awash_books_store';
...
const Home: React.FC<{
  ...
  return (
    <>
      ...
      {!props.isLoading && props.newArrivalsBooks.length > 0 && (
        <NewArrivals 
                ...
                <div className={`rounded-3xl p-6 mb-8 border transition-colors duration-300 shadow-sm flex flex-col sm:flex-row sm:items-center justify-between gap-4 ${props.isDarkMode ? 'bg-slate-900 border-slate-800' : 'bg-white border-slate-200'}`}>
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    >
                      ...
                      <option value="year-newest">አዲስ (በዓመተ ምህረት)</option>
                      <option value="year-oldest">የቆዩ (በዓመተ ምህረት)</option>
        ...
        <div className="flex flex-col md:flex-row gap-12">
          <div className="md:w-1/3">
            ...
            <div className="mt-6 flex flex-col gap-3">
              ...
              <button 
                onClick={() => props.onShare(book)}
                className={`w-full py-3.5 flex items-center justify-center gap-2 rounded-2xl font-bold transition-all border ${
                  props.isDarkMode 
...
const App: React.FC = () => {
  ...
  // Initialize and persist allBooks
  useEffect(() => {
    const savedBooks = localStorage.getItem(BOOKS_STORE_KEY);
    if (savedBooks) {
      try {
        setAllBooks(JSON.parse(savedBooks));
      } catch (e) {
  ...
  useEffect(() => {
    if (allBooks.length > 0) {
      localStorage.setItem(BOOKS_STORE_KEY, JSON.stringify(allBooks));
    }
  }, [allBooks]);
  ...
  const newArrivalsBooks = useMemo(() => {
    return allBooks.filter(b => b.isNew);
  }, [allBooks]);
  ...
  useEffect(() => {
    ...
    if (savedReadLater) {
      try {
        ...
        setReadLaterBooks(allBooks.filter(b => ids.includes(b.id)));
    ...
    if (savedRecs) {
      try {
        ...
        setAiRecommendations(allBooks.filter(b => ids.includes(b.id)));
      } catch (e) { console.error(e); }
    }
  ...
  const fetchAIRecommendations = async () => {
    ...
    try {
      ...
      const catalog = allBooks.map(b => ({ id: b.id, title: b.title, description: b.description, category: b.category }));
      ...
      const recommendedBooks = allBooks.filter(b => recommendedIds.includes(b.id)).slice(0, 4);
  ...
  const filteredAndSortedBooks = useMemo(() => {
    ...
    switch (sortBy) {
      ...
      case 'year-newest': result.sort((a, b) => (Number(b.year) || 0) - (Number(a.year) || 0)); break;
      case 'year-oldest': result.sort((a, b) => (Number(a.year) || 0) - (Number(b.year) || 0)); break;
    }
    return result;
  }, [allBooks, activeCategory, searchQuery, sortBy, priceRange, selectedAuthors]);
      ...
      <div className={`min-h-screen transition-colors duration-300 ${isDarkMode ? 'bg-slate-950 text-slate-100' : 'bg-slate-50 text-slate-900'} flex flex-col font-['Inter','Noto_Sans_Ethiopic',sans-serif]`}>
        ...
        <header className={`sticky top-0 z-40 transition-colors duration-300 ${isDarkMode ? 'bg-slate-900/90 border-slate-800' : 'bg-white/90 border-slate-200'} backdrop-blur-md border-b`}>
          <div className="max-w-7xl mx-auto px-4 h-20 flex items-center justify-between">
            ...
            <div className="hidden lg:flex items-center gap-6 ml-6 text-sm font-bold uppercase tracking-wider">
              <Link to="/" className={`transition-colors ${isDarkMode ? 'text-slate-400 hover:text-white' : 'text-slate-600 hover:text-blue-600'}`}>መነሻ</Link>
              <Link to="/about" className={`transition-colors ${isDarkMode ? 'text-slate-400 hover:text-white' : 'text-slate-600 hover:text-blue-600'}`}>ስለ እኛ</Link>
              <Link to="/admin" className={`flex items-center gap-1.5 transition-colors px-3 py-1.5 rounded-lg ${isDarkMode ? 'bg-slate-800 text-blue-400 hover:text-blue-300' : 'bg-blue-50 text-blue-600 hover:text-blue-700'}`}>
        ...
        {previewBook && (
          <BookPreview 
            ...
            isDarkMode={isDarkMode} 
            isBookmarked={bookmarkedIds.includes(previewBook.id)}
        ...
        {isReadLaterOpen && (
          <ReadLaterModal 
            ...
            bookmarkedBooks={allBooks.filter(b => bookmarkedIds.includes(b.id))}
        ...
        <footer className={`transition-colors duration-300 py-16 mt-20 ${isDarkMode ? 'bg-slate-900 text-slate-400 border-t border-slate-800' : 'bg-slate-900 text-slate-300'}`}>
          <div className="max-w-7xl mx-auto px-4">
            <div className="grid grid-cols-1 md:grid-cols-4 gap-12 mb-12">
              <div className="col-span-1 md:col-span-2">
                <Link to="/" className="flex items-center gap-3 mb-6">
                  <div className="w-10 h-10 bg-blue-600 rounded-xl flex items-center justify-center text-white text-2xl font-bold">A</div>
                  <h2 className="text-2xl font-bold text-white">አዋሽ ዲጅታል</h2>
                </Link>
                <p className="max-w-md mb-6 leading-relaxed">
                  አዋሽ ዲጂታል መጻሕፍት መደብር የኢትዮጵያን እና የዓለምን ምርጥ መጻሕፍት ለሁሉም ተደራሽ ለማድረግ የሚሰራ ዘመናዊ የዲጂታል መድረክ ነው። 
                  ባህልን እና ቴክኖሎጂን በማጣመር ንባብን እናቀላጥፋለን።
                </p>
                <div className="flex gap-4">
                  {['FB', 'TG', 'X', 'IG'].map(s => (
                    <button key={s} className="w-10 h-10 rounded-full bg-slate-800 flex items-center justify-center hover:bg-blue-600 hover:text-white transition-all">{s}</button>
                  ))}
                </div>
              </div>
              <div>
                <h3 className="text-white font-bold uppercase tracking-widest mb-6">ፈጣን ሊንኮች</h3>
                <ul className="space-y-4 text-sm font-medium">
                  <li><Link to="/" className="hover:text-blue-500 transition-colors">መነሻ ገጽ</Link></li>
                  <li><Link to="/about" className="hover:text-blue-500 transition-colors">ስለ እኛ</Link></li>
                  <li><Link to="/admin" className="hover:text-blue-500 transition-colors">አስተዳደር</Link></li>
                  <li><button onClick={() => window.scrollTo({top: 0, behavior: 'smooth'})} className="hover:text-blue-500 transition-colors">መጻሕፍትን ያስሱ</button></li>
                </ul>
              </div>
              <div>
                <h3 className="text-white font-bold uppercase tracking-widest mb-6">እገዛ እና ድጋፍ</h3>
                <ul className="space-y-4 text-sm font-medium">
                  <li><button className="hover:text-blue-500 transition-colors">የአጠቃቀም መመሪያ</button></li>
                  <li><button className="hover:text-blue-500 transition-colors">የግላዊነት ፖሊሲ</button></li>
                  <li><button className="hover:text-blue-500 transition-colors">ያግኙን</button></li>
                  <li><button className="hover:text-blue-500 transition-colors">ተዘውታሪ ጥያቄዎች</button></li>
                </ul>
              </div>
            </div>
            <div className="pt-8 border-t border-slate-800 text-center">
              <p className="text-xs">© 2025 አዋሽ ዲጅታል መጻሕፍት መደብር። መብቱ በህግ የተጠበቀ ነው።</p>
            </div>
          </div>
        </footer>
      </div>
